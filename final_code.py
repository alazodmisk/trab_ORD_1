# TRABALHO ORGANIZAÇÃO E RECUPERAÇÃO DE DADOS - I
...
class ChaveSecundaria:
    def __init__(self, nome: str, indice: int):
        self.nome = nome 
        self.indice = indice #Primeiro caso de *nome*        
...
def busca_indice_primario(listas: list[ChavePrincipal], argumento: int, games: io.BufferedRandom):
    '''A função recebe um ID como argumento e realiza uma busca binária no índice
    primário (localizado na primeira posição da lista de índices) para localizar
    esse registro. Se encontrado, ela obtém o offset armazenado no índice,
    posiciona o ponteiro do arquivo nessa posição e lê os primeiros 2 bytes para
    descobrir o tamanho do registro. Em seguida, lê a quantidade exata de bytes,
    decodifica o texto e exibe o registro na tela com um traço na frente.
    
    Exibe mensagem informando qual ID está sendo buscado
    - Extrai a lista de índice primário da posição 0 do parâmetro listas
    - Executa busca binária pelo ID no índice primário
    - Se não encontrar: exibe "Registro não encontrado!" e encerra
    - Se encontrar: obtém o offset do registro a partir da chave primária
    - Posiciona o arquivo no offset, lê o tamanho (2 bytes) e o registro completo, então exibe'''
...
def busca_indice_secundario(listas:list[list[ChavePrincipal]|list[ChaveSecundaria]|list[Indices]], codigo: int, argumento: str, games: io.BufferedRandom):
    '''Esta função busca todos os registros que correspondem a uma chave secundária, que pode ser gênero *código 1* ou publicadora *código 2*. Ela utiliza o índice
    secundário para localizar a chave e a lista invertida para percorrer os registros
    relacionados. Cada registro encontrado é lido do arquivo binário e exibido na tela. Se a chave não existir, uma mensagem de erro apropriada é mostrada.
        
    Busca a chave no índice secundário
        Se não achar, exibe erro e encerra
        Se achar, percorre a lista invertida seguindo os ponteiros (.proxGenero ou .proxPublicadora)
        Para cada nó, localiza o offset no índice primário e lê o registro do arquivo
...
def compactar_arquivo():
    '''Remove logicamente os registros marcados como excluídos do arquivo games.dat.

    Esta função percorre o arquivo games.dat, identifica registros que foram
    marcados para remoção (ID começando com "") e os exclui fisicamente,
    gerando um novo arquivo compactado apenas com os registros válidos.

    Comportamento:
    
    - Abre o arquivo games.dat em modo leitura binária
    - Lê cada registro a partir do seu tamanho (2 primeiros bytes)
    - Decodifica o registro e separa os campos pelo caractere "|"  
    - Se for válido, armazena o registro em uma lista temporária
    - Se começar com "*", ignora (exclusão lógica)
    - Cria um arquivo temporário com apenas os registros válidos
    - Substitui o games.dat original pelo arquivo compactado usando os.replace()'''
    registro:list[str] = []
    validos:list = []
    ...
    with open('temporarios.dat', 'wb') as gamesCompactados:
        for registro in validos:
            tam = len(registro).to_bytes(2, 'little')
            gamesCompactados.write(tam)
            gamesCompactados.write(registro)

    os.replace('temporarios.dat', 'games.dat')

def executar_operacoes(nome_arquivo):
    '''Executa operações de busca, inserção e remoção a partir de um arquivo de comandos.
    Esta função carrega todos os índices (primário, secundários e lista invertida) dos
    seus respectivos arquivos binários, abre o arquivo de dados "games.dat" e processa
    as operações linha por linha. Cada comando do arquivo é identificado e executado:
    'bp' para busca por ID, 'bs1' para busca por gênero, 'bs2' para busca por publicadora,
    'i' para inserção e 'r' para remoção. Ao final, todos os índices são reescritos nos
    arquivos para persistir as alterações.

        Comportamento:
        
    - Carrega os quatro índices dos arquivos: primario.ind, genero.ind, publicadora.ind e listaInvertida.lst
    - Abre o arquivo games.dat para leitura e escrita
    - Lê cada linha do arquivo de operações e pula linhas vazias
    - Para cada linha, separa o comando do argumento e executa a função correspondente
    - Se o comando não for reconhecido, exibe mensagem de erro
    - Após todas as operações, salva os índices atualizados nos respectivos arquivos
    - Em caso de arquivo de operações não encontrado, exibe erro'''
...
def main():
    ...
    if flag == '-b':
        ...
        compactar_arquivo()
        construir_indices()
        print("Arquivo 'games.dat' compactado. Índices reconstruídos.")
    else:
